...
async def update_problem(database: Database = Depends(get_connection)):
    ...
    query_raw = 'INSERT INTO problems (id, title, text, correct_ans) VALUES (:id, :title, :text, :correct_ans) ON CONFLICT ON CONSTRAINT "problems_id_key" DO UPDATE SET title = :title, text = :text, correct_ans = :correct_ans'
    # values = [
